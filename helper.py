...
class Helper:
    ...
    def get_txtpathlist(self, dirpath='/media/gu/Three/大三下/InformationRetrieval/2014/'):
        """
         获取txt全部的路径
        :param dirpath:
        :return:
    ...
    def combine_alltext(self, pathlist):
        ...
        :return:
        """
        wf = open('documents/all_text.txt', 'w+')
        for path in pathlist[:4]:
            txt = open(path, 'r')
            text = txt.read()
            wf.write(text)
            wf.write('\n')
    ...
    def get_word_tag(self):
        ...
        """
        rf = open('documents/all_text.txt', 'r')
        word_tag_list = []
        lines = rf.readlines()
        for line in lines:
            line = line.replace("\n", "").replace("\r", "")
    ...
    def get_emission_probability(self, word_tag_list):
        ...
        wf = open('documents/emission_probability.txt', 'w+')
    ...
    def get_transition_probability(self, tag_tag_list):
        ...
        :return:
        """
        wf = open('documents/transition_probability.txt','w+')
        start_pro = self.load_start_profortransemi()
        fdist = nltk.FreqDist(tag_tag_list)
        print fdist.items()
        for key, value in fdist.items():
    ...
    def load_start_profortransemi(self, path='documents/start_probability.txt'):
        """
    ...
    def load_words(self, path='documents/words.txt'):
        """
        把语料的字全部加载进来，用于初始化方法
        :param path:
        :return:
    ...
    def load_start_pro(self, path='documents/start_probability.txt'):
        """
    ...
    def load_transition_pro(self, path='documents/transition_probability.txt'):
        """
    ...
    def load_emission_pro(self, path='documents/emission_probability.txt'):
        """
